...
async def get_company(agent_api_key: str) -> dict:
    try:
        async with agent_tool_context(agent_api_key, family="agent_self") as (session, agent):
            ...
            if payload is None:
                raise parity_http_error(404, "Agent does not have an active company")
            return {"ok": True, "company": payload}
    except Exception as exc:
        return handle_tool_error(exc)
...
async def get_company_buildings(agent_api_key: str) -> dict:
    try:
        async with agent_tool_context(agent_api_key, family="agent_self") as (session, agent):
            try:
                payload = await get_agent_company_buildings(session, agent.id)
            except ValueError as exc:
                raise parity_http_error(404, str(exc)) from exc
            return {"ok": True, "buildings": payload}
    except Exception as exc:
        return handle_tool_error(exc)
